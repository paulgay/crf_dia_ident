...
public class ACRFTrainer {
    ...
    public boolean train (ACRF acrf,
			  ...
			  int numIter)
    {
	Maximizable.ByGradient macrf = createMaximizable (acrf, trainingList);//ACRF.java l985
	return train (acrf, trainingList, validationList, testSet,
		      eval, numIter, macrf);//150
    }
    ...
    */
	...
	for (int i = 0; i < numIter; i++) {
	    ...
	    try {
			converged = maximizer.maximize (macrf, 1);//gives new value to parameters
	//		converged |= callEvaluator (acrf, trainingList, validationList, testSet, i, eval); can't evaluate using the default function because the labels are not consistent accross instances, anyway, the log evaluator always return true
			if (converged) break;
			resetOnError = true;
	    ...
	    if (converged) break;
	    // "early" stopping
	    currentValue = macrf.getValue ();// reinitialisation of the expectations with the new parameters
	...
	((MaximizableACRF)macrf).printParameters();
	acrf.print(System.out);
	if (testSet != null) {
//	    eval.test (acrf, testSet, "Testing");  
//		List ret = acrf.getBestLabels (testSet);
//		System.out.println(ret); 
	}
	//Template 0 =0.012749036294449493	TEMPLATE 1=9.56097069482054E-4	TEMPLATE 2=7.377481418685219E-5	TEMPLATE 3 =7.183425519535407E-5	TEMPLATE 4=-1.4599641072212134E-5	TEMPLATE 5=-2.0035901790718695E-5
	//double[] params = {0,0,0,0,0,0,4.1908012879180205,2.3435000428825465,0.00664978217562369,0.06701744044415384,-0.02653260450834734,-0.01513423633350917};
	//((MaximizableACRF)macrf).setParameters(params);
	//double log=((MaximizableACRF)macrf).computeLogLikelihood ();
	//System.out.print("log likelihood"+log);
	//acrf.print(System.out);
	return converged;
    }
