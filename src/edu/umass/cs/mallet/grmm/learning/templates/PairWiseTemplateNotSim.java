...
public class PairWiseTemplateNotSim extends ACRF.ContinuousTemplate{
	...
	protected int initDenseWeights (InstanceList training){
		...
		features.add(features.get(0)+"_diff");
		features.set(0, features.get(0)+"_same");
		features.add("hung");
	...
	public  AbstractTableFactor computeFactor (UnrolledVarSet clique){
    	...
    	for (int loc = 0; loc < phi.numLocations(); loc++) {
    		...
    		//big mess
    		String seg1=segmentNames.get(seg1Idx);
    		String vlabel,alabel,avlabel;
    		if(seg1.matches("_")){
    			vlabel=outcome1;
    			alabel=outcome2;
    		}
        	...
        	for(int i=0;i<features.size();i++){
        		if(features.get(i).equals("hung"))
        			if(hungLabels.contains(avlabel))
        				values[i]=1;
        			else
        				values[i]=0;
        		else{
