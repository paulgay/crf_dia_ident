...
public class InstanceFactory2  extends AbstractPipeInputIterator{
	...
	String aScoreFile,avScoreFile,vSiftScoreFile,featFile,uniqFile,vDctScoreFile;
	String aScoreDir,avScoreDir,vSiftScoreDir,featDir,uniqDir,outputDir,listDir;
    public InstanceFactory2(String configfile,int iterNumber, ArrayList<String> shows, ACRF.Template[] templates)throws Exception{
    	String line, pattern="[ ]+";
    	String[] tokens;
	...
	vSiftScoreFile=vSiftScoreDir+"/"+currentShow+".vsifttable";
        vDctScoreFile=vSiftScoreDir+"/"+currentShow+".vdcttable";
	...
	try{
	    ...
	    if(iterNumber==0){
	    	addNoise(aScoreFile,carrier); // put some noise in the biometrics models of the train. (labels which are not mapped by a reference but which are produced by the diarization. they are introduced for the CRF to learn how to get rid of them) concretely adds the biometric labels in labelIdx (label dictionnary) if not present
	    ...
	    for(int i=0;i<templates.length;i++){
	    	...
	    	if(templates[i] instanceof UniquenessTemplate){
                ...
                if(templates[i] instanceof VisualTemplateDct){
                  System.out.println(vDctScoreFile);
                  carrier.setVCostTable(getVScore2(vDctScoreFile,carrier,0.86,false));
                  carrier.dumpVCostTable(iterNumber);
                }
	    }
	    carrier.setName(currentShow);
	    carrier.setDir(this.dir);
